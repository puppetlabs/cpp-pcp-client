...
The [JsonContainer][3] class is used frequently by the cpp-pcp-client library as a
...
The first step to interacting with a PCP server is creating a connection. To
...
The parameters are described as:
 ...
 a connection to the PCP server if a problem occurs. It will try to connect
...
Every message sent over the PCP server has to specify a value for the _data_schema_
field in the message envelope. These data schema's determine how a message's data
section is validated. To process messages received from a PCP server you must
...
Once you have established a connection to the PCP server you can send messages
...
received from the PCP server will be matched against a Schema that you defined.
